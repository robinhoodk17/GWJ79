...
@export var max_roll : Vector3 = Vector3(0.2,0.2,.5) #Maximum rotation in radians (use sparingly).
...
func add_trauma(amount : float, max_trauma : float):
	trauma = min(trauma + amount, max_trauma)


func add_kaiju_step_trauma(amount : float, max_trauma : float):
	kaiju_trauma = min(kaiju_trauma + amount, max_trauma)


func shake(howMuch : float):
	var amt = pow(howMuch, trauma_pwr)
	noise_y += 1
...
func kaiju_shake(howMuch : float, max_trauma : float):
	var amt = pow(howMuch, kaiju_trauma_pwr)
	kaiju_noise_y += 1
	var rollz = max_roll.z * amt * noise.get_noise_2d(30,kaiju_noise_y)
	var rotatingBasis : Basis = camera_nest.basis
	rotatingBasis = rotatingBasis.rotated(camera_nest.basis.z,rollz)
	camera.basis =	rotatingBasis


func handleShakes(delta):
	if trauma:
